...
@Component({
  selector: 'ngx-event-reservation-report',
  templateUrl: './event-reservation-report.component.html',
  styleUrls: ['./event-reservation-report.component.scss']
})
export class EventReservationReportComponent implements OnInit {
  Data:any={};
  file:any;
  filepath:any;
  fileData=null;
  ...
  ngOnInit() {
    this.showpage=true
    this.reportsService.EventReservationData().subscribe(res=>{this.totalData=res.Data;
      this.showpage=false
      })
     this.getData();
  }
  ...
  getData(){
    ...
    this.reportsService.EventReservationGetList(this.pagingparamater).subscribe(
      res => {
  ...
  filter(filter: string) {
   ...
   Export() {
    this.reportsService.ExportEventReservation(this.pagingparamater).subscribe(res => {
      let path = res.Data;
