...
export class CountriesComponent implements OnInit {
  ...
  onEdit(e){
    this.addEditaddressModel = this.modalService.show(AddCountryComponent, { initialState:
  ...
  onDeleteConfirm(event): void {
    ...
    this.warningModel.content.onClose = (cancel) => {
      if (cancel) {
        this.countryService.Delete(event.data.id).subscribe(res=>{
          if(res.Success){
  ...
  getData(){
    this.countryService.GetList().subscribe(res=>{this.source.load(res.Data);this.Data=res.Data;;
    });
  }
  addNew() {
    this.addEditaddressModel = this.modalService.show(AddCountryComponent, { initialState:
