...
export class CitiesComponent implements OnInit {
  ...
  onEdit(e){
    this.addEditaddressModel = this.modalService.show(AddCityComponent, { initialState:
  ...
  onDeleteConfirm(event): void {
    ...
    this.warningModel.content.onClose = (cancel) => {
      if (cancel) {
        this.jobService.Delete(event.data.id).subscribe(res=>{
          if(res.Success){
  ...
  getData(){
    this.jobService.GetList().subscribe(res=>{this.source.load(res.Data);this.Data=res.Data;;
    });
  }
  addNew() {
    this.addEditaddressModel = this.modalService.show(AddCityComponent, { initialState:
